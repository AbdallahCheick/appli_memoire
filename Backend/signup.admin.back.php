<?php
    if(isset($_POST['sub_inp']))
    {
        ...
        {
            header("Location: ../Frontend/admin/index.php?error=emptyfields&uid=".$user."&mail=".$email);
            exit();
        }
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $user))
        {
            header("Location: ../Frontend/admin/index.php?error=invalidmailuid");
            exit();
        }
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL))
        {
            header("Location: ../Frontend/admin/index.php?error=invalidmail&uid=".$user);
            exit();
        }
        else if (!preg_match("/^[a-zA-Z0-9]*$/", $user))
        {
            header("Location: ../Frontend/admin/index.php?error=invaliduid&mail=".$email);
            exit();
        }
        else if ($password !== $passwordRepeat)
        {
            header("Location: ../Frontend/admin/index.php?error=passwordcheck&uid=".$user."&mail=".$email);
            exit();
        }
        else
        {
            ...
            {
                header("Location: ../Frontend/admin/index.php?error=sqlerror");
                exit();
            }
            else
            {
                ...
                if ($resultCheck > 0)
                {
                    header("Location: ../Frontend/admin/index.php?error=usertaken&mail=".$email);
                    exit();
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        header("Location: ../Frontend/admin/index.php?signup=success");
                        exit();
                    }
                }
            }
