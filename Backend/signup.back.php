<?php
    if(isset($_POST['sub_inp']))
    {
        ...
        {
            header("Location: ../Frontend/login.php?error=emptyfields&uid=".$user."&mail=".$email);
            exit();
        }
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $user))
        {
            header("Location: ../Frontend/login.php?error=invalidmailuid");
            exit();
        }
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL))
        {
            header("Location: ../Frontend/login.php?error=invalidmail&uid=".$user);
            exit();
        }
        else if (!preg_match("/^[a-zA-Z0-9]*$/", $user))
        {
            header("Location: ../Frontend/login.php?error=invaliduid&mail=".$email);
            exit();
        }
        else if ($password !== $passwordRepeat)
        {
            header("Location: ../Frontend/login.php?error=passwordcheck&uid=".$user."&mail=".$email);
            exit();
        }
        else
        {
            ...
            else
            {
                ...
                if ($resultCheck > 0)
                {
                    header("Location: ../Frontend/login.php?error=usertaken&mail=".$email);
                    exit();
                }
                else
                {
