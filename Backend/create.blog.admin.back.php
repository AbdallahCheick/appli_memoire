...
if (isset($_POST['create_blog'])) {
    ...
    if (empty($title) || empty($content)) {
        header('Location: ../Frontend/admin/blogs.admin.php?error=emptyfields');
        exit();
    } else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            header(
                'Location: ../Frontend/admin/blogs.admin.php?error=sqlerror'
            );
            exit();
        } else {
            ...
            if ($resultCheck > 0) {
                header(
                    'Location: ../Frontend/admin/blogs.admin.php?error=titletaken'
                );
                exit();
            } else {
                $id = $_SESSION['blog_id'];
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    header(
                        'Location: ../Frontend/admin/blogs.admin.php?catcreation=success'
                    );
                }
            }
        }
