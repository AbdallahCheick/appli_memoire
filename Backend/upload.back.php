    ...
    if (in_array($fileActualExt, $allowed)) {
        if ($fileError === 0) {
            if ($fileSize < 10000000) {
                ...
                header(
                    'Location: ../Frontend/signup.php?error=imgsizeexceeded'
                );
                exit();
            }
        } else {
            header('Location: ../Frontend/signup.php?error=imguploaderror');
            exit();
        }
    } else {
        header('Location: ../Frontend/signup.php?error=invalidimagetype');
        exit();
    }
}
